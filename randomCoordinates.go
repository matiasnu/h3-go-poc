...
func RandLocation(location Location, radius float64) Location {
	len := math.Sqrt(rand.Float64()) * radius
	deg := rand.Float64() * 2 * math.Pi
	x := location.Latitude + (len * math.Cos(deg))
	y := location.Longitude + (len * math.Sin(deg))
	return Location{
		Latitude:  x,
		Longitude: y,
	}
